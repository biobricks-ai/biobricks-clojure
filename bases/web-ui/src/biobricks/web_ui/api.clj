  ...
  {::ds/defs
   {:brick-data
    {:brick-db
     ...
     :local-datalevin
     (datalevin/local-db-component
      {:dir "datalevin"
       :schema (ds/local-ref [:datalevin-schema])})}
    :web-ui
    {:app
     (sys/config-component
      {:brick-db (ds/ref [:brick-data :brick-db])
       :datalevin-conn (ds/ref [:brick-data :local-datalevin :conn])})}}})
...
(defn start-system! []
  (swap! system #(sys/start! (or % (system-def)))))

(defn stop-system! []
  (swap! system sys/stop!))

(defn restart-system! []
  (swap! system #(do
                   (when % (sys/stop! %))
                   (sys/start! (system-def)))))
