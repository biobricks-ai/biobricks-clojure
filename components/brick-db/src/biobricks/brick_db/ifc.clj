...
(defn datalevin-schema []
  (-> "brick-db/datalevin-schema.edn" io/resource io/reader PushbackReader. edn/read))

(defn- run-every-ms [interval-ms f ex-handler]
...
(defn check-github-repos [{:keys [datalevin-conn github-org-name]}]
  (doseq [{:keys [created_at full_name html_url id updated_at]} (github/list-org-repos github-org-name)]
    (dtlv/transact!
     datalevin-conn
...
(defn github-poller [{:as instance :keys [github-poll-interval-ms]}]
  (doto (Thread. #(run-every-ms
...
(defn component [& {:keys [datalevin-conn github-org-name github-poll-interval-ms]}]
  {::ds/config
   ...
   (fn [{::ds/keys [instance]
     ...
     (if-not datalevin-conn
       ...
       (do
         ...
         (dissoc instance :datalevin-conn :github-org-name :github-poll-interval-ms :github-poller))))})
